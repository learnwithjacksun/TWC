        ...
        <div className="main">
          ...
          <div className="my-6 flex flex-col md:flex-row gap-4 md:gap-8">
                      <div className="flex-1 rounded-md border border-line overflow-hidden">
                          <img src={project?.image&& storage.getFilePreview("images", project?.image).href} alt="Project Image" />
                      </div>
                      <div className="flex-1 flex flex-col gap-6">
                          <div className="flex flex-col gap-2">
                              <h2 className="text-xl font-bold font-sora">{project?.title}</h2>
                              <p className="text-sub text-sm font-medium">By {project?.name} <span className="text-xs font-semibold bg-green-500/10 border-green-500  border px-2 rounded-full text-green-500 inline-flex">Developer</span></p>
                          ...
                          <div>
                              ...
                              <ul className="flex flex-wrap gap-2 mt-2">
                                  {project?.tools.map((x, y) => (
                                      <li key={y} className="text-sm px-4 py-2 rounded-full bg-lighter border border-line">
                                          {x}
                                      </li>
                                  ))}
                              </ul>
